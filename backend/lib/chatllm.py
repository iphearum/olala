...
llm = OpenAI(
    api_base="http://localhost:11434/v1", api_token="openkh", model_name="openkh"
)
...
class AIChat:
            ...
            [
                {
                    k[0]: k[1]
                    for k in [
                        i.split(": ") for i in node.get_metadata_str().split("\n")
                    ]
                },
                node.get_content(),
            ]
    ...
    def chat(
        ...
        if len(messages) > 0:
            return self.ai.chat(
                model=chatllm_model, messages=messages, stream=stream, keep_alive=-1
            )
    ...
    def translate(
        self, text: str, translator="google", from_language="auto", to_language="km"
    ):
    ...
    async def texttospeech(self, text: str, sound="male"):
        sounds = await requests.post(
            "https://tts-api.idri.edu.kh/api/v1/text-to-speech",
            data={"text": text, "sound": sound},
        )
        return sounds
